...
const cleanUp = () =>
  fetchAsAdmin(`/users/${__e2e.testUserCredentials.email}`, {
    method: "DELETE"
  })
    .then(resp => {
        ...
        .then(json =>
          Promise.all(
            (Array.isArray(json) ? json : [])
              ...
              .map(product =>
                fetchAsAdmin(`/products/${product._id}`, { method: "DELETE" })
              )
          )
        )
        .then(responses => {
        ...
        .then(json =>
          Promise.all(
            (Array.isArray(json) ? json : [])
              .filter(order =>
                order.items.reduce(
                  (memo, item) => memo || item.name === "Test",
                  false
                )
              )
              .map(order =>
                fetchAsAdmin(`/orders/${order._id}`, { method: "DELETE" })
              )
          )
        )
        .then(responses => {
